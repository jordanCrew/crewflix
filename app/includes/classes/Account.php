...
class Account {
    ...
    public function login($un, $pw){
        ...
        $query = $this->con->prepare("SELECT * FROM users WHERE username=:un AND pword=:pw");
    ...
    private function insertUserDetails($fn, $ln, $un, $em, $pw) {
        ...
        $query = $this->con->prepare("INSERT INTO users (firstName, lastName, username, email, pword)
    ...
    public function updatePassword($oldPw, $pw, $npw2, $un ) {
        $this->validateOldPassword($oldPw, $un);
        $this->validatePasswords($pw, $npw2);

            if(empty($this->errorArray)){
            $query =$this->con->prepare("UPDATE users SET pword=:pw
                                        WHERE username=:un");
            $pw = hash("sha512", $pw);
